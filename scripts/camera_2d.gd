...
func _ready() -> void:
	...
	#position.x = _subject.position.x + following_offset.x * (-1 if _subject._is_facing_left else 1)
...
func _process(delta: float) -> void:
		...
		if _subject.is_on_floor():
			...
			if _subject.velocity.y > 0:
				_vertical_tween = create_tween()
				_vertical_tween.tween_property(self, "_vertival_movement" , _subject.position.y + following_offset.y ,0.7)
				self.position.y = _vertival_movement
			else:
				_vertical_tween = create_tween()
				_vertical_tween.tween_property(self, "_vertival_movement" , _subject.position.y + following_offset.y ,1)
